...
def test_data_setup(docker_services, source_database_id):
    ...
    root_collection_id = source.create_collection(
        ...
        description="Root collection for integration tests"
    )
    ...
    card4_id = source.create_card(
        ...
        query={
            ...
            "type": "query",
            "query": {
                "source-table": f"card__{card1_id}"
            }
        }
    )
...
def db_map_file(tmp_path, source_database_id, target_database_id):
    ...
    db_map = {
        "by_id": {
            str(source_database_id): target_database_id
        },
        "by_name": {
            "Sample Data": target_database_id
        }
    }
...
class TestEndToEndExportImport:
    ...
    def test_export_from_source(self, docker_services, test_data_setup, export_dir, source_database_id):
    ...
    def test_import_to_target(self, docker_services, test_data_setup, export_dir,
                             db_map_file, source_database_id, target_database_id):
        """Test importing data to target Metabase."""
        # First run export
        self.test_export_from_source(docker_services, test_data_setup, export_dir, source_database_id)
    ...
    def test_dry_run_import(self, docker_services, test_data_setup, export_dir,
                           db_map_file, source_database_id):
        ...
        assert len(final_collections) == initial_collection_count, "Dry run should not create collections"

    def test_export_with_dependencies(self, docker_services, test_data_setup, export_dir, source_database_id):
    ...
    def test_conflict_strategy_skip(self, docker_services, test_data_setup, export_dir,
                                   db_map_file, source_database_id):
        """Test import with skip conflict strategy."""
        # First import
        self.test_import_to_target(docker_services, test_data_setup, export_dir,
                                  db_map_file, source_database_id, None)
        ...
        assert len(collections_after_second) == first_import_count, "Skip strategy should not create duplicates"
...
class TestMultipleInstances:
    ...
    def test_different_database_ids(self, docker_services, source_database_id, target_database_id):
        ...
        assert source_database_id != target_database_id or source_database_id == target_database_id
    ...
    def test_independent_instances(self, docker_services):
        ...
        source_collection_id = source.create_collection(
            name="Source Only Collection",
            description="This should only exist in source"
        )
