...
class TestCardDependencyExtraction:
    ...
    def test_no_dependencies(self, sample_export_config):
        ...
        deps = CardExporter.extract_card_dependencies(card_data)
        assert deps == set()
...
class TestDependencyChainTracking:
    ...
    def test_track_simple_dependency_chain(self, sample_export_config):
        ...
        # Dependency chain is now handled as a parameter in export_card_with_dependencies
        # This test verifies the concept still works
        dependency_chain = [50, 100]

        assert 50 in dependency_chain
        assert 100 in dependency_chain
...
class TestCircularDependencyDetection:
    ...
    def test_detect_direct_circular_dependency(self, sample_export_config):
        ...
        # Dependency chain is now handled as a parameter
        dependency_chain = [50, 51]

        # If we try to add 50 again, it would be circular
        is_circular = 50 in dependency_chain
...
class TestDependencyOrdering:
    ...
    def test_dependencies_exported_before_dependent(self, sample_export_config):
        ...
        # Exported cards are now tracked in CardExporter
        exported_cards = {50, 51}

        # Card 100 depends on 50 and 51
        # Both should already be exported
        assert 50 in exported_cards
        assert 51 in exported_cards
...
class TestComplexDependencyScenarios:
    ...
    def test_diamond_dependency(self, sample_export_config):
        ...
        deps_a = CardExporter.extract_card_dependencies(card_a)
        deps_b = CardExporter.extract_card_dependencies(card_b)
        deps_c = CardExporter.extract_card_dependencies(card_c)
        deps_d = CardExporter.extract_card_dependencies(card_d)
    ...
    def test_long_dependency_chain(self, sample_export_config):
        ...
        for i, card in enumerate(cards):
            deps = CardExporter.extract_card_dependencies(card)
            if i == 0:
                assert deps == set()
            else:
