...
class MetabaseImporter:
    ...
    def __init__(self, config: ImportConfig):
        ...
        self.db_map: Optional[DatabaseMap] = None
    ...
    def _resolve_db_id(self, source_db_id: int) -> Optional[int]:
                ...
                if target_db_id is None:
                    if card.database_id not in unmapped:
                        unmapped[card.database_id] = UnmappedDatabase(
                            ...
                            source_db_name=self.manifest.databases.get(str(card.database_id), "Unknown Name")
                        )
    ...
    def _validate_target_databases(self):
        ...
        try:
            ...
            if missing_ids:
                ...
                logger.error("=" * 80)
                logger.error("Your db_map.json references database IDs that don't exist in the target instance.")
                logger.error("")
                ...
                logger.error("2. Make sure you're mapping to databases that exist in the target instance")
                logger.error("=" * 80)
                sys.exit(1)
    ...
    def _perform_dry_run(self):
        ...
        if unmapped_dbs:
            ...
            for db in unmapped_dbs:
                logger.error(f"  - Source DB ID: {db.source_db_id} (Name: '{db.source_db_name}') is not mapped.")
                logger.error(f"    Used by cards (IDs): {', '.join(map(str, sorted(list(db.card_ids))[:10]))}{'...' if len(db.card_ids) > 10 else ''}")
        ...
        for card in sorted(self.manifest.cards, key=lambda c: c.file_path):
             if card.archived and not self.config.include_archived: continue
        ...
        if self.manifest.dashboards:
            ...
            for dash in sorted(self.manifest.dashboards, key=lambda d: d.file_path):
                if dash.archived and not self.config.include_archived: continue
    ...
    def _perform_import(self):
        ...
        logger.info(f"Collections: {summary['collections']['created']} created, {summary['collections']['updated']} updated, {summary['collections']['skipped']} skipped, {summary['collections']['failed']} failed.")
        logger.info(f"Cards: {summary['cards']['created']} created, {summary['cards']['updated']} updated, {summary['cards']['skipped']} skipped, {summary['cards']['failed']} failed.")
        if self.manifest.dashboards:
            logger.info(f"Dashboards: {summary['dashboards']['created']} created, {summary['dashboards']['updated']} updated, {summary['dashboards']['skipped']} skipped, {summary['dashboards']['failed']} failed.")
        
        report_path = self.export_dir / f"import_report_{datetime.datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _find_target_collection_by_path(self, source_path: str) -> Optional[Dict]:
        ...
        for part in path_parts:
            ...
            for target_coll in self._target_collections:
                if (sanitize_filename(target_coll['name']) == part and 
                    target_coll.get('parent_id') == current_parent_id):
    ...
    def _import_collections(self):
        ...
        for collection in tqdm(sorted_collections, desc="Importing Collections"):
            try:
                target_parent_id = self._collection_map.get(collection.parent_id) if collection.parent_id else None
                ...
                if existing_coll:
                    ...
                    self.report.add(ImportReportItem("collection", "skipped", collection.id, existing_coll['id'], collection.name, "Exists on target"))
                    continue

                payload = {"name": collection.name, "description": collection.description, "parent_id": target_parent_id}
                
                new_coll = self.client.create_collection(clean_for_create(payload))
                self._collection_map[collection.id] = new_coll['id']
                self.report.add(ImportReportItem("collection", "created", collection.id, new_coll['id'], collection.name))

            except Exception as e:
                logger.error(f"Failed to import collection '{collection.name}': {e}")
                self.report.add(ImportReportItem("collection", "failed", collection.id, None, collection.name, str(e)))
    ...
    def _topological_sort_cards(self, cards: List) -> List:
        ...
        if len(sorted_cards) < len(cards):
            remaining = [card_map[card_id] for card_id in card_map.keys() if card_id not in [c.id for c in sorted_cards]]
            ...
            for card in remaining:
                ...
                if missing_deps:
                    logger.warning(f"Card {card.id} ('{card.name}') depends on missing cards: {missing_deps}")
    ...
    def _remap_card_query(self, card_data: Dict) -> Tuple[Dict, bool]:
        ...
        if not source_db_id: return data, False

        target_db_id = self._resolve_db_id(source_db_id)
        if not target_db_id:
            raise ValueError(f"FATAL: Unmapped database ID {source_db_id} found during card import. This should have been caught by validation.")
            ...
            if isinstance(source_table, str) and source_table.startswith("card__"):
                try:
                    ...
                    if source_card_id in self._card_map:
                        ...
                        logger.debug(f"Remapped source-table from card__{source_card_id} to card__{self._card_map[source_card_id]}")
                except ValueError:
                    logger.warning(f"Invalid card reference format: {source_table}")
                ...
                if isinstance(join_source_table, str) and join_source_table.startswith("card__"):
                    try:
                        ...
                        if source_card_id in self._card_map:
                            ...
                            logger.debug(f"Remapped join source-table from card__{source_card_id} to card__{self._card_map[source_card_id]}")
                    except ValueError:
                        logger.warning(f"Invalid card reference in join: {join_source_table}")
    ...
    def _import_cards(self):
        ...
        cards_to_import = [card for card in self.manifest.cards
                          if not card.archived or self.config.include_archived]
        ...
        for card in tqdm(sorted_cards, desc="Importing Cards"):
            try:
                ...
                # 2. Remap collection
                target_collection_id = self._collection_map.get(card.collection_id) if card.collection_id else None
                ...
                self.report.add(ImportReportItem("card", "created", card.id, new_card['id'], card.name))
                ...
                if "does not exist" in error_msg and "Card" in error_msg:
                    ...
                    if match:
                        ...
                        logger.error(f"The card references another card (ID: {missing_card_id}) that doesn't exist in the target instance.")
                        logger.error("")
                        ...
                        logger.error(f"3. Card {missing_card_id} is archived and --include-archived was not used during export")
                        logger.error("")
                        logger.error("SOLUTIONS:")
                        logger.error(f"1. Re-export with card {missing_card_id} included")
                        logger.error("2. If the card is archived, use --include-archived flag during export")
                        logger.error("3. Manually create or import the missing card first")
                        logger.error("=" * 80)
                        self.report.add(ImportReportItem("card", "failed", card.id, None, card.name,
                                                        f"Missing dependency: card {missing_card_id}"))
                        continue

                # Check for table ID constraint violation
                elif "fk_report_card_ref_table_id" in error_msg.lower() or ("table_id" in error_msg.lower() and "not present in table" in error_msg.lower()):
                    ...
                    logger.error("=" * 80)
                    logger.error(f"Failed to import card '{card.name}' (ID: {card.id})")
                    logger.error(f"The card references table ID {table_id} that doesn't exist in the target Metabase instance.")
                    logger.error("")
                    logger.error("This is a known limitation: Table IDs are instance-specific and cannot be directly migrated.")
                    logger.error("")
                    logger.error("CAUSE:")
                ...
                elif "FK_REPORT_CARD_REF_DATABASE_ID" in error_msg or "FOREIGN KEY(DATABASE_ID)" in error_msg:
                    logger.error("=" * 80)
                    logger.error("❌ DATABASE MAPPING ERROR DETECTED!")
                    logger.error("=" * 80)
                    logger.error(f"Failed to import card '{card.name}' (ID: {card.id})")
                    logger.error(f"The card references database ID {card.database_id}, but this database ID does not exist in the target Metabase instance.")
                    logger.error("")
                    logger.error("This means your db_map.json file is incorrectly configured.")
                    logger.error("")
                    logger.error("SOLUTION:")
                    logger.error("1. Check your db_map.json file")
                    logger.error("2. Ensure the source database ID is mapped to a valid target database ID")
                    logger.error("3. You can list target databases using: GET /api/database")
                    logger.error("")
                    logger.error(f"Source database ID: {card.database_id}")
                    logger.error(f"Source database name: {self.manifest.databases.get(str(card.database_id), 'Unknown')}")
                ...
                # Check for field ID constraint violation
                elif "fk_query_field_field_id" in error_msg.lower() or "field_id" in error_msg.lower():
                    logger.error("=" * 80)
                    logger.error("❌ FIELD ID MAPPING ERROR DETECTED!")
                    logger.error("=" * 80)
                    logger.error(f"Failed to import card '{card.name}' (ID: {card.id})")
                    logger.error("The card references field IDs that don't exist in the target Metabase instance.")
                    logger.error("")
                    logger.error("This is a known limitation: Field IDs are instance-specific and cannot be directly migrated.")
                    logger.error("")
                    logger.error("SOLUTIONS:")
                    logger.error("1. Recreate the card manually in the target instance")
                    logger.error("2. Use native SQL queries instead of GUI queries (they use field names, not IDs)")
                    ...
                    logger.error(f"Failed to import card '{card.name}' (ID: {card.id}): {e}", exc_info=True)
                    self.report.add(ImportReportItem("card", "failed", card.id, None, card.name, str(e)))
    ...
    def _import_dashboards(self):
        ...
        for dash in tqdm(sorted(self.manifest.dashboards, key=lambda d: d.file_path), desc="Importing Dashboards"):
            ...
            try:
                ...
                # 1. Remap collection
                target_collection_id = self._collection_map.get(dash.collection_id) if dash.collection_id else None
                            ...
                            for param_mapping in dashcard["parameter_mappings"]:
                                ...
                                if "card_id" in clean_param:
                                    ...
                                    if source_param_card_id in self._card_map:
                                        clean_param["card_id"] = self._card_map[source_param_card_id]
                            ...
                            for series_card in dashcard["series"]:
                                if isinstance(series_card, dict) and "id" in series_card:
                                    ...
                                    if series_card_id in self._card_map:
                                        ...
                                        clean_dashcard["series"].append({"id": self._card_map[series_card_id]})
                                    else:
                ...
                for param in parameters:
                    ...
                    if "values_source_config" in param_copy and isinstance(param_copy["values_source_config"], dict):
                        source_card_id = param_copy["values_source_config"].get("card_id")
                        if source_card_id:
                            if source_card_id in self._card_map:
                                # Remap to target card ID
                                param_copy["values_source_config"]["card_id"] = self._card_map[source_card_id]
                                logger.debug(f"Remapped parameter card_id {source_card_id} -> {self._card_map[source_card_id]}")
                            else:
                                # Card not found, skip this parameter
                                logger.warning(f"Skipping dashboard parameter '{param.get('name')}' - references missing card {source_card_id}")
                                continue
                ...
                if dashcards_to_import:
                    ...
                    logger.debug(f"Updating dashboard {new_dash['id']} with {len(dashcards_to_import)} dashcards")
                    ...
                    for idx, dc in enumerate(dashcards_to_import):
                        for field in problematic_fields:
                            if field in dc:
                                logger.error(f"Dashcard {idx} still has '{field}' field: {dc.get(field)} - this will cause import to fail!")
                ...
                self.report.add(ImportReportItem("dashboard", "created", dash.id, updated_dash['id'], dash.name))

            except Exception as e:
                logger.error(f"Failed to import dashboard '{dash.name}' (ID: {dash.id}): {e}", exc_info=True)
                self.report.add(ImportReportItem("dashboard", "failed", dash.id, None, dash.name, str(e)))
